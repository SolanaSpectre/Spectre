...
function normalizeDossierTrade(entry, exit) {
  ...
  const record = {
    ...
    paperLane: paper.lane || entryPaper.lane || null,
    paperProfile: paper.profileName || entryPaper.profileName || paper.exitProfile?.profileName || entryPaper.exitProfile?.profileName || null,
    ...
    walletClassificationContext: entryPaper.walletClassificationContext || paper.walletClassificationContext || null,
...
function collectContinuationTrades(statePath) {
  ...
  return asArray(state.positions).map((position) => {
    ...
    const record = {
      ...
      entryMode: position.entryMode || null,
      guardOverride: null,
...
function buildPatterns(trades, minPatternCount) {
  ...
  for (const trade of trades) {
    const keys = [
      `lane:${trade.lane}`,
      trade.paperLane ? `paper_lane:${trade.paperLane}` : null,
      trade.preset ? `preset:${trade.preset}` : null,
      trade.paperProfile ? `profile:${trade.paperProfile}` : null,
      trade.entryMode ? `entry_mode:${trade.entryMode}` : null,
