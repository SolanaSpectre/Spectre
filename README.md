...
- `PRE_MIGRATION_PAPER_CURVE_FALSE_NEGATIVE_BRIDGE_*`: Strategy research bridge for wallet-supported `CURVE_NOT_ADVANCING` false-negative candidates. Current stalled-curve bridge paper entries are paused by default (`PRE_MIGRATION_PAPER_CURVE_FALSE_NEGATIVE_BRIDGE_PAPER_ENTRIES_ENABLED=false`) after negative PAPER5 evidence. The recovery shadow remains report-only and requires a tracked pre-85 wallet buy, no qualifying-wallet sell after that buy, curve recovery confirmation, provider/on-chain curve parity, bounded score/curve/volume/velocity/buy-ratio thresholds, and a small per-run cap if paper entries are explicitly re-enabled. This preset does not enable live execution or broadcast.
...
- `PRE_MIGRATION_PAPER_SAME_MINT_REENTRY_COOLDOWN_MS`: Short mint-level cooldown after any pre-migration paper exit to prevent immediate same-mint churn across presets (default: `120000`)
