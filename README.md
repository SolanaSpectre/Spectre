...
- `PRE_MIGRATION_OBSERVED_TELEMETRY_MIN_SCORE_DELTA`: Score movement that bypasses observed telemetry throttling (default: `1`)
- `PRE_MIGRATION_OBSERVED_TELEMETRY_MIN_CURVE_DELTA`: Curve-progress movement that bypasses observed telemetry throttling (default: `0.005`)
...
- `PRE_MIGRATION_PAPER_CURVE_FALSE_NEGATIVE_BRIDGE_*`: Strategy research bridge for wallet-supported `CURVE_NOT_ADVANCING` false-negative candidates. Current stalled-curve bridge paper entries are paused by default (`PRE_MIGRATION_PAPER_CURVE_FALSE_NEGATIVE_BRIDGE_PAPER_ENTRIES_ENABLED=false`) after negative evidence. If paper entries are explicitly re-enabled, they now require the same proof family as the recovery shadow by default: tracked pre-85 wallet buy, no qualifying-wallet sell after that buy, curve recovery confirmation, provider/on-chain curve parity, bounded score/curve/volume/velocity/buy-ratio thresholds, and a small per-run cap. This preset does not enable live execution or broadcast.
...
- `PRE_MIGRATION_PAPER_EARLY_ACCELERATION_BLOCK_WEAK_WALLET_FLOW`: Blocks early-acceleration paper entries when wallet context is dominated by low-signal touches and a late sell, without changing live execution (default: `true`)
- `PRE_MIGRATION_PAPER_EARLY_ACCELERATION_BLOCK_AVOID_WALLET_CONTEXT`: Blocks early-acceleration paper entries when tracked wallet context includes avoid/negative evidence, without changing live execution (default: `true`)
- `PRE_MIGRATION_PAPER_EARLY_ACCELERATION_WEAK_WALLET_FLOW_MIN_LOW_SIGNAL_TOUCHES`: Low-signal wallet touches required before the weak-flow guard can block an early-acceleration paper entry (default: `3`)
- `PRE_MIGRATION_PAPER_EARLY_ACCELERATION_WEAK_WALLET_FLOW_MIN_LATE_SELL_SOL`: Late-sell SOL needed before the weak-flow guard can block an early-acceleration paper entry (default: `1`)
- `PRE_MIGRATION_PAPER_HIGH_CONVICTION_FIRST_SIGHT_REQUIRE_WALLET_CONTEXT`: Blocks high-conviction first-sight paper entries that have no tracked wallet context, without changing live execution (default: `true`)
...
- `PRE_MIGRATION_PAPER_SAME_MINT_REENTRY_COOLDOWN_MS`: Mint-level cooldown after any pre-migration paper exit to prevent same-mint churn across presets (default: `1800000`)
- `PRE_MIGRATION_PAPER_BLOCK_AVOID_WALLET_CONTEXT`: Blocks pre-migration paper entries when tracked wallet context includes avoid/negative evidence, without changing live execution (default: `true`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_REQUIRE_WALLET_CONTEXT`: Blocks late-curve pre-migration paper entries with no tracked wallet context, without changing live execution (default: `true`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_REQUIRE_WALLET_CONTEXT_MIN_CURVE_PROGRESS`: Curve-progress threshold where the late-curve wallet-context guard begins applying (default: `0.88`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_WALLET_QUALITY_GUARD_ENABLED`: Blocks high-curve paper entries when wallet proof shows sell-after-first-touch, low-signal first positive touch, or sniper crowding (default: `true`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_WALLET_QUALITY_MIN_CURVE_PROGRESS`: Curve-progress threshold where late wallet-quality checks begin applying (default: `0.9`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_WALLET_QUALITY_BLOCK_POSITIVE_SELL_AFTER_BUY`: Blocks high-curve paper entries after a positive/proven wallet sells following its first buy (default: `true`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_WALLET_QUALITY_BLOCK_LOW_SIGNAL_FIRST_TOUCH`: Blocks high-curve paper entries when the first positive/proven touch is low-signal/insufficient-data labeled (default: `true`)
- `PRE_MIGRATION_PAPER_HIGH_CURVE_WALLET_QUALITY_MAX_SNIPER_WALLET_COUNT`: Blocks high-curve wallet-backed paper entries above this sniper-wallet count (default: `7`)
- `SHADOW_WALLET_FILE_PATH`: Optional report-only untracked wallet profile file. Shadow wallets are logged separately under `shadowWallets` and do not satisfy real wallet-proof entry guards (default: `data/wallet-watchlists/shadow-untracked-wallets.json`)
- `PRE_MIGRATION_PAPER_RECHECK_REASONS`: Comma-separated pre-migration paper skip reasons that schedule bounded on-chain rechecks for fresher curve state (default: `NO_PRIOR_CURVE_PROGRESS,CURVE_NOT_ADVANCING`)
- `PRE_MIGRATION_CURVE_FALSE_NEGATIVE_SHADOW_REPLAY_MAX_FILES`: Recent telemetry files used by the report-only curve false-negative shadow replay, so one good run does not overrule the broader sample (default: `9`)
