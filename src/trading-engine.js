  ...
  observePreMigrationToken(token, launchIntelSummary = null) {
    ...
    if (shouldEmitWatchTelemetry) {
      this.telemetry.record(result.flagged ? 'pre_migration.flagged' : 'pre_migration.observed', {
        ...
        providerCurveProgress: result.state.providerCurveProgress ?? result.state.curveProgress,
        providerCurvePriceSol: result.state.providerCurvePriceSol ?? result.state.bondingCurvePriceSol,
